...
# edited: 5/2/18, 6/27/18, 6/28/18/ 7/5
...
g <- function(x, plot_num, alpha = 0.5, caption = NULL){
    # error check
    ...
    ggplot(data = x, aes(x = date.time))+
        ...
        labs(title = "Processed dendrometer data",
             ...
             x = "date", y = "growth (mm)", caption = caption)
}

#histograms
g_hist <- function(x, facet, title = NULL, subtitle = NULL, xlim = c(-1,3), 
                   binwidth = 0.1, mm_growth = "mm_growth"){
    ggplot(data = x)+
        geom_histogram(aes_string(x = mm_growth, y = "..density.."), binwidth = binwidth)+
...
dendro_means <- function(x, var = NULL, other_group = NULL, mm_adj = FALSE){
    ...
    if(!mm_adj){
        ...
        x <-  x %>%
            ...
            summarize(
                mm_growth = mean(mm_growth, na.rm = T),
                mm_growth_adj = mean(mm_growth_adj, na.rm = T)
            ) %>%
            ungroup() 
    }
...
# mm_adj arg is for if there is a second column of adjusted values to average
...
# TRUE for is.na, false for !is.na based check_NA arg

check_isna <- function(z, check_NA){
    if (!is.logical(check_NA)){ 
        stop("check_NA needs to be T/F")
    }
...
na_range <- function(x, y,...){
    #args:
    # x-- time vector
    # y -- vector to check for NAs in, 
    # output:
    # lengths of sections of NAS and there positions with respect to x
    
    #~~~
    given = list(...)
    default = list(gap_length = 48, check_NA = TRUE)
    args <- modifyList(default, given)
    gap_length = args[["gap_length"]]
    check_NA <- args[["check_NA"]]
    #~~~
    ...
    r <- rle(check_isna(df_in$y, check_NA = check_NA))
    lengths <- r[["lengths"]]
    values <- r[["values"]]
...
avg_ends <- function(x, y1, y2,...){
    ...
    range <- na_range(df$x, df$y2, check_NA = TRUE,...)
    # range_y1 <- na_range(df$x, df$y1, check_NA = FALSE)
